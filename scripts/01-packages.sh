...
yum -y install @core authconfig audit deltarpm sudo chrony cloud-init \
  cloud-utils-growpart dracut-config-generic dracut-norescue firewalld \
  grub2 kernel nfs-utils rsync tar yum-utils tuned dnsmasq gnutls gdisk \
  libselinux-python libyaml lsof lvm2 m2crypto libmcrypt \
  ...
  python-requests python-six python-urllib3 python-zmq PyYAML \
  screen unzip vim-common vim-enhanced wget yum-utils zeromq3 zip
